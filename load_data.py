...
def load_data_subset(data_aug, batch_size,workers,dataset, data_target_dir, labels_per_class=100, valid_labels_per_class = 500, random_label=False):
    ...
    #print ('svhn', train_data.labels.shape)
        ...
        validation_root = os.path.join(data_target_dir, 'val')  # this is path to validation images folder/images
    ...
    if random_label:
        import random
        _targets = train_data.targets 
        train_data.targets = random.shuffle(_targets)
...
def load_data_subset_with_deform(data_aug, 
                                 batch_size,
                                 workers,
                                 dataset, 
                                 data_target_dir, 
        ...
        #                                         [transforms.ToTensor(), transforms.Normalize(mean, std)])
            ...
            train_transform = transforms.Compose([
                                                ...
                                                #   transforms.RandomCrop(32, padding=2),
                                                  ...
                                                  transforms.Normalize(mean, std)])
            test_transform = transforms.Compose(
                                                [transforms.ToTensor(), transforms.Normalize(mean, std)]+deform_transforms)
    else:
        print ('no data aug')
        if dataset == 'mnist':
            hw_size = 28
